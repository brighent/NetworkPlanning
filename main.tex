...
\hl{As previously stated, {\ac{ce}} training provides {\cite{Bishop2006}}}
\begin{equation}
	\tilde{y} \sim p(\mathcal{H}_0|\bm y).	
\end{equation}
From the Bayes rule, 
\begin{equation}
	p(\mathcal{H}_0| \bm y) = \sigma\left( \mathcal{L}^{(\bm y)} + \log \frac{p(\mathcal{H}_0)}{p(\mathcal{H}_1)} \right).	
\end{equation}
Hence, thresholding $\tilde{y}$ is equivalent to thresholding $\mathcal{L}^{(\bm y)}$, as $\sigma(\cdot)$ is an invertible function. From \eqref{eq:decNN} we can then obtain the \ac{roc}.
