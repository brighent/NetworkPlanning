...
\begin{equation}
\label{eq:final}
	\lim_{S \to \infty}	\hatcross{\bm p_{\mathcal{H}|\bm y}}{g} =	\E{ \cross{\bm p_{\mathcal{H}|\bm y}}{\bm q}} = 
		\E{H (\bm p_{\mathcal{H}|\bm y}) + D(\bm p_{\mathcal{H}|\bm y}||\bm q)},
\end{equation}
where $D(\cdot||\cdot)$ is the Kullback-Leibler divergence, $H_{\bm c}(\bm d)$ is the cross entropy function between the two discrete \acp{pmd} $\bm c$ and $\bm d$, $H(\cdot)$ is the entropy function and $\E{\cdot}$ is the expected value with respect to the distribution of $\bm y$. The second equality holds by definition of cross entropy.
...
However, at convergence the \ac{nn} achieves the same performance of the NP approach. This holds since \eqref{eq:decNN} is equivalent to \eqref{eq:thrOpt}, with
\begin{equation}
\label{eq:relation}
